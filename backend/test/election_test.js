...
describe('Update Function Test', () => {
  ...
  afterEach(() => {
    sandbox.restore(); // <-- cleans every stub/spy made with this sandbox
  });
  ...
  it('should update election successfully', async () => {
    ...
    sandbox.stub(Election, 'findById').resolves(existingElection);

    const req = {
      params: { id: electionId },           // or electionId.toString()
      body: { title: 'New Election', completed: true },
    };
    const res = {
      json: sandbox.spy(),
  ...
  it('should return 404 if election is not found', async () => {
    sandbox.stub(Election, 'findById').resolves(null);
    ...
    expect(res.status.calledWith(404)).to.be.true;
    // Using calledWithMatch is a bit more forgiving on object equality
    sinon.assert.calledWithMatch(res.json, { message: 'Election name not found' });
  });

  it('should return 500 on error', async () => {
    // Use rejects for async failures
    sandbox.stub(Election, 'findById').rejects(new Error('DB Error'));
